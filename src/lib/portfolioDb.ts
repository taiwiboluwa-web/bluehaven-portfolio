...
export async function readPublicPortfolio() {
  ...
  return data.projects.filter((project) => project.visible).map((project) => ({ ...project, media: (mediaByProject.get(project.id) || []).sort((a, b) => Number(Boolean(b.featured)) - Number(Boolean(a.featured)) || a.sort_order - b.sort_order) }));
}
...
export async function addMedia(input: { id: string; project_id: string; storage_url: string; storage_key: string; alt_text: string; sort_order: number; featured: boolean; file_name: string; mime_type: string }) {
  ...
  await sql`INSERT INTO portfolio_media(id, project_id, storage_url, storage_key, alt_text, media_type, sort_order, featured, created_at, updated_at, file_data, file_name, mime_type) VALUES(${input.id}, ${input.project_id}, ${input.storage_url}, ${input.storage_key}, ${input.alt_text}, 'image', ${input.sort_order}, ${input.featured}, NOW(), NOW(), NULL, ${input.file_name}, ${input.mime_type}) ON CONFLICT(id) DO UPDATE SET project_id=EXCLUDED.project_id, storage_url=EXCLUDED.storage_url, storage_key=EXCLUDED.storage_key, alt_text=EXCLUDED.alt_text, sort_order=EXCLUDED.sort_order, featured=EXCLUDED.featured, file_name=EXCLUDED.file_name, mime_type=EXCLUDED.mime_type, updated_at=NOW()`;
}
...
export async function deleteProject(id: string) {
  const sql = getDb();
  await sql`DELETE FROM portfolio_media WHERE project_id=${id}`;
