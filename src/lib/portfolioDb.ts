...
export type PortfolioMedia = { id: string; project_id: string; storage_url: string; storage_key: string | null; alt_text: string; media_type: 'image'; sort_order: number; featured: boolean; created_at?: string; updated_at?: string; file_name?: string | null; mime_type?: string | null };
...
export async function readPortfolio() {
  ...
  const [projectRows, mediaRows] = await Promise.all([
    ...
    sql`SELECT id, project_id, storage_url, storage_key, alt_text, media_type, sort_order, featured, created_at, updated_at, file_name, mime_type FROM portfolio_media ORDER BY project_id ASC, sort_order ASC, created_at ASC`,
  ]);
...
export async function createProject(input: Omit<PortfolioProject, never>) {
  const sql = getDb();
  await sql`INSERT INTO portfolio_projects(id, slug, name, category, description, website_url, visible, sort_order, gallery_layout, created_at, updated_at) VALUES(${input.id}, ${input.slug}, ${input.name}, ${input.category}, ${input.description}, ${input.website_url}, ${input.visible}, ${input.sort_order}, ${JSON.stringify({ aspectRatio: input.gallery_layout })}::jsonb, ${input.created_at}, ${input.updated_at})`;
}
...
export async function toggleProject(id: string) {
  const sql = getDb();
  const rows = await sql`UPDATE portfolio_projects SET visible=NOT visible, updated_at=NOW() WHERE id=${id} RETURNING id`;
  if (!rows[0]) throw new Error('Project not found');
}
...
export async function addMedia(input: { id: string; project_id: string; storage_url: string; storage_key: string; alt_text: string; sort_order: number; featured: boolean; file_name: string; mime_type: string }) {
  const sql = getDb();
  ...
  await sql`
    INSERT INTO portfolio_media(id, project_id, storage_url, storage_key, alt_text, media_type, sort_order, featured, created_at, updated_at, file_data, file_name, mime_type)
    VALUES(${input.id}, ${input.project_id}, ${input.storage_url}, ${input.storage_key}, ${input.alt_text}, 'image', ${input.sort_order}, ${input.featured}, NOW(), NOW(), NULL, ${input.file_name}, ${input.mime_type})
    ON CONFLICT(id) DO UPDATE SET project_id=EXCLUDED.project_id, storage_url=EXCLUDED.storage_url, storage_key=EXCLUDED.storage_key, alt_text=EXCLUDED.alt_text, sort_order=EXCLUDED.sort_order, featured=EXCLUDED.featured, file_name=EXCLUDED.file_name, mime_type=EXCLUDED.mime_type, updated_at=NOW()
  `;
}

export async function updateMediaUrl(id: string, storageUrl: string, storageKey: string, mimeType: string) {
  const sql = getDb();
  const rows = await sql`UPDATE portfolio_media SET storage_url=${storageUrl}, storage_key=${storageKey}, mime_type=${mimeType}, updated_at=NOW() WHERE id=${id} RETURNING id`;
  if (!rows[0]) throw new Error('Media not found');
}

export async function getMedia(id: string) {
  const sql = getDb();
  const rows = await sql`SELECT id, project_id, storage_url, storage_key, alt_text, media_type, sort_order, featured, created_at, updated_at, file_name, mime_type FROM portfolio_media WHERE id=${id} LIMIT 1`;
...
export async function deleteMediaRecord(id: string) {
  const sql = getDb();
  const rows = await sql`DELETE FROM portfolio_media WHERE id=${id} RETURNING project_id`;
  if (!rows[0]) throw new Error('Media not found');
}

export async function deleteProject(id: string) {
  const sql = getDb();
  const rows = await sql`DELETE FROM portfolio_projects WHERE id=${id} RETURNING id`;
  if (!rows[0]) throw new Error('Project not found');
}
