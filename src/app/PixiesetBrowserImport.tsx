...
export default function PixiesetBrowserImport() {
  ...
  useEffect(() => {
    const receive = async (event: MessageEvent) => {
      ...
      let capturedOrigin = '';
      try { capturedOrigin = new URL(String(event.data.url || '')).origin; } catch { return; }
      if (event.origin !== capturedOrigin || !/pixieset\.com$/i.test(new URL(event.data.url).hostname)) return;
      setBusy(true);
      ...
      try {
        const response = await fetch('/api/pixieset', {
          ...
          body: JSON.stringify({ action: 'browser_preview', source_url: event.data.url, browser_snapshot: event.data }),
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data.error || 'The browser capture could not be read.');
        setPreview(data.preview);
        setSnapshotHtml(String(event.data.html || ''));
  ...
  const openGallery = () => {
    try { new URL(url); } catch { setMessage('Enter a valid Pixieset URL first.'); return; }
    setPreview(null);
    setSnapshotHtml('');
  ...
  const copyCaptureScript = async () => {
    try {
      ...
      setMessage('Could not copy the helper. Copying requires clipboard permission in this browser.');
    }
  };

  const importGallery = async () => {
    if (!preview || !url || !snapshotHtml) return;
    setBusy(true);
    ...
    try {
      const response = await fetch('/api/pixieset', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ action: 'browser_import', source_url: url, browser_snapshot: { type: 'bluehaven-pixieset-snapshot', url, html: snapshotHtml }, name, category, description, visible: true, gallery_layout: 'landscape' }),
      });
      const data = await response.json().catch(() => ({}));
  ...
  return <div className="fixed bottom-5 right-5 z-50 w-[min(420px,calc(100vw-2rem))] overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#111114] shadow-2xl shadow-black/60">
    ...
    <div className="space-y-3 p-5">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[.03] p-3 text-[11px] leading-5 text-white/50"><strong className="text-white/75">When the server gets 403:</strong> open the gallery, then paste the copied <code className="text-[#ffde59]">javascript:</code> helper into the Pixieset address bar and run it. It sends only the public page currently rendered in your browser back to this admin tab. No password/private collection is bypassed.</div>
      ...
      {preview&&<div className="rounded-xl bg-black/20 p-3"><p className="font-bold">{preview.title}</p><p className="mt-1 text-xs text-white/45">{preview.photos.length} images found{preview.expectedPhotoCount&&preview.expectedPhotoCount>preview.photos.length?` of ${preview.expectedPhotoCount} advertised`:''}.</p><div className="mt-2 grid grid-cols-5 gap-1">{preview.photos.slice(0,5).map(photo=><img key={photo.url} src={photo.url} alt="" className="aspect-square rounded object-cover" loading="lazy"/>)}</div><div className="mt-3 space-y-2"><input className={input} value={name} onChange={e=>setName(e.target.value)} placeholder="Project title"/><input className={input} value={category} onChange={e=>setCategory(e.target.value)} placeholder="Category"/><textarea className={`${input} min-h-20`} value={description} onChange={e=>setDescription(e.target.value)} placeholder="Description"/><button type="button" onClick={importGallery} disabled={busy||!snapshotHtml} className="w-full rounded-xl bg-[#ffde59] px-3 py-2 text-xs font-bold text-black disabled:opacity-40">Import this gallery into Work</button></div></div>}
    </div>
  </div>;
}
