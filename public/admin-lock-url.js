(() => {
  ...
  styles.textContent = `
    ...
    .section-card--locked .layout-controls,
    .section-card--locked .section-content,
    .section-card--locked .section-visible { pointer-events:none; opacity:.48; }
    ...
    .bluehaven-url-dialog h2 { margin:0 0 8px; }
    .bluehaven-url-dialog p { color:#999; margin:0 0 18px; }
    .bluehaven-url-dialog label { display:grid; gap:8px; color:#bbb; }
    .bluehaven-url-dialog input { width:100%; box-sizing:border-box; }
    .bluehaven-url-actions { display:flex; gap:10px; justify-content:flex-end; margin-top:18px; }
  `;
  ...
  function lockSections() {
    document.querySelectorAll('.section-card[data-section]').forEach(card => {
      ...
      card.querySelectorAll('input,select,textarea,button').forEach(control => {
        if (control.closest('.section-title') && control.classList.contains('switch')) return;
  ...
  function makeMediaCard(media) {
    ...
    card.querySelector('.delete-media').onclick = async () => {
      ...
      try {
        await api({ action: 'deleteMedia', id: media.id });
        card.remove();
        document.querySelector(`.gallery-tile[data-media-order="${CSS.escape(media.id)}"]`)?.remove();
        toast('Image removed.');
  ...
  function makeGalleryTile(media) {
    ...
    tile.innerHTML = `<img src="${esc(media.url)}" alt="${esc(media.alt)}"><span>${String(index).padStart(2, '0')}</span>`;
    preview.appendChild(tile);
  }

  function openUrlDialog({ logo = false } = {}) {
    const existing = document.querySelector('.bluehaven-url-modal');
    existing?.remove();
    const modal = document.createElement('div');
    modal.className = 'bluehaven-url-modal';
    ...
    form.onsubmit = async event => {
      ...
      if (!urlOkay(url)) return toast('Enter a valid public http(s) image URL.');
      const id = projectId();
      if (!id) return toast('Open a project first.');
      ...
      try {
        if (logo) {
          ...
          if (preview) preview.innerHTML = `<img src="${esc(url)}" alt="Project logo">`;
          toast('Project logo URL saved.');
        } else {
          const name = (() => { try { return new URL(url).pathname.split('/').filter(Boolean).pop()?.replace(/[-_]+/g, ' ').replace(/\.[^.]+$/, '') || 'Image'; } catch { return 'Image'; } })();
          const image = { action: 'saveMedia', projectId: id, url, alt: name, type: 'image', featured: false };
          const data = await api(image);
          if (!data.media) throw Error('The server did not return the saved image.');
          makeGalleryTile(data.media);
          makeMediaCard(data.media);
  ...
  function enhanceMedia() {
    ...
    if (add && !document.querySelector('#add-media-url')) {
      const button = document.createElement('button');
      button.id = 'add-media-url';
      button.type = 'button';
      button.className = 'secondary small url-media-button';
      button.textContent = '＋ Add image URL';
      button.title = 'Paste a public image URL';
      button.onclick = () => openUrlDialog();
      add.parentElement?.appendChild(button);
    }
    const logoActions = document.querySelector('#upload-project-logo')?.parentElement;
    if (logoActions && !document.querySelector('#logo-url-button')) {
      const button = document.createElement('button');
      button.id = 'logo-url-button';
      button.type = 'button';
      button.className = 'secondary small';
      button.textContent = 'Use logo URL';
      button.onclick = () => openUrlDialog({ logo: true });
      logoActions.appendChild(button);
    }
  }

  document.addEventListener('click', event => {
    if (event.target.closest('[data-section-locked]')) {
      const blocked = event.target.closest('input,select,textarea,button');
