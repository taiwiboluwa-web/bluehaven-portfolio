...
function authenticated(req:Request){const raw=req.headers.get('cookie')?.match(/(?:^|;\s*)bluehaven_admin=([^;]+)/)?.[1];if(!raw)return false;const parts=raw.split('.');if(parts.length!==3)return false;try{return timingSafeEqual(Buffer.from(parts[2]),Buffer.from(sign(`${parts[0]}.${parts[1]}`)))}catch{return false}}
