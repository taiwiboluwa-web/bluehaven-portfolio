...
// Keep the deployment configuration backward-compatible while using the
// BlueHaven-specific variable already configured in Vercel.
const secret = () =>
  process.env.BLUEHAVEN_ADMIN_PASSWORD || process.env.ADMIN_PASSWORD || '';
...
export default async function handler(req: VercelRequest) {
  ...
  if (!secret()) {
    return json(
      { error: 'BLUEHAVEN_ADMIN_PASSWORD is not configured on the deployment.' },
      503,
    );
  }
