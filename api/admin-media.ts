...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const body = req.body || {};
      const [project] = await sql()(`INSERT INTO portfolio_projects (slug,name,category,description,website_url,visible,sort_order) VALUES (${body.slug},${body.name},${body.category || null},${body.description || null},${body.websiteUrl || null},${body.visible !== false},${Number(body.sortOrder || 0)}) ON CONFLICT (slug) DO UPDATE SET name=EXCLUDED.name, category=EXCLUDED.category, description=EXCLUDED.description, website_url=EXCLUDED.website_url, visible=EXCLUDED.visible, sort_order=EXCLUDED.sort_order, updated_at=NOW() RETURNING *`);
