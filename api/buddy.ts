...
const sign = (value: string) =>
  createHmac('sha256', process.env.ADMIN_PASSWORD || '').update(value).digest('base64url');

function auth(req: Request) {
  const raw = req.headers.get('cookie')?.match(/(?:^|;\s*)bluehaven_admin=([^;]+)/)?.[1];
...
const normalize = (value: unknown): 'bubbles' | 'skales' =>
  value === 'skales' ? 'skales' : 'bubbles';
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    return json({ error: String(error).replace('Error: ', '') }, 503);
  }
  ...
  try {
    ...
    await db`
      ...
      VALUES ('active_buddy', ${JSON.stringify(activeBuddy)}::jsonb, NOW())
      ON CONFLICT (setting_key)
      DO UPDATE SET setting_value=${JSON.stringify(activeBuddy)}::jsonb, updated_at=NOW()
    `;
