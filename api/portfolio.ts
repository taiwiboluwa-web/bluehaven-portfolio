...
function auth(req:VercelRequest){const raw=cookieHeader(req).match(/(?:^|;\s*)bluehaven_admin=([^;]+)/)?.[1];if(!raw||!process.env.ADMIN_PASSWORD)return false;const p=raw.split('.');if(p.length!==3)return false;try{return timingSafeEqual(Buffer.from(p[2]),Buffer.from(sign(`${p[0]}.${p[1]}`)))}catch{return false}}
const bodyOf=(req:VercelRequest)=>{if(req.body&&typeof req.body==='object')return req.body as Record<string,unknown>;if(typeof req.body==='string'){try{return JSON.parse(req.body)}catch{return {}}}return {}};
...
export default async function handler(req:VercelRequest){
 let db;try{db=sql()}catch(e){return req.method==='GET'&&new URL(req.url||'/',`https://${req.headers?.host||'localhost'}`).searchParams.get('mode')==='public'?json({projects:[],fallback:true}):json({error:String(e).replace('Error: ','')},503)}
 const params=new URL(req.url||'/',`https://${req.headers?.host||'localhost'}`).searchParams;
 if(req.method==='GET'&&params.get('mode')==='public'){try{return json({projects:await visible(db)})}catch{return json({projects:[],fallback:true})}}
