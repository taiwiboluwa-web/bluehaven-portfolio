...
export default async function handler(req,res){
  ...
  if(req.method==='POST' && req.url.split('?')[0].endsWith('/api/admin.js')){
    ...
    if(data.action==='login'){
      ...
      setAuth(res); return res.status(200).json({ok:true});
    }
  ...
  try{
    ...
    if(data.action==='saveSection'){
      ...
      const current=(await sql`SELECT layout,content,visible,sort_order FROM site_sections WHERE id=${data.id}`)[0];
      if(!current)return res.status(404).json({error:'Section not found.'});
      const layout=data.layout===undefined?current.layout:{...(current.layout||{}),...(data.layout||{})};
      const content=data.content===undefined?current.content:{...(current.content||{}),...(data.content||{})};
      await sql`UPDATE site_sections SET visible=${data.visible===undefined?current.visible:Boolean(data.visible)},sort_order=${data.sortOrder===undefined?current.sort_order:Number(data.sortOrder)||0},layout=${layout},content=${content},updated_at=now() WHERE id=${data.id}`;
      return res.status(200).json({ok:true});
    }
    if(data.action==='saveSetting'){
      if(!data.key)return res.status(400).json({error:'Setting key is required.'});
      await sql`INSERT INTO site_settings(setting_key,setting_value,updated_at) VALUES(${data.key},${data.value||{}},now()) ON CONFLICT(setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value,updated_at=now()`;
      return res.status(200).json({ok:true});
    }
    if(data.action==='reorderSections'){
      const order=Array.isArray(data.ids)?data.ids:[];
      for(let i=0;i<order.length;i++) await sql`UPDATE site_sections SET sort_order=${(i+1)*10},updated_at=now() WHERE id=${order[i]}`;
      return res.status(200).json({ok:true});
    }
